...
每次傳不同的 `-resource`，AuthGate 就會發出兩張 `aud` 不同的 JWT。

---

## 前置條件

1. **AuthGate** 已跑在 `http://localhost:8080`，並完成 GitHub（或 Gitea /
   Microsoft）federation 設定。
2. **註冊一個 OAuth client** in AuthGate（或啟用 DCR 由 client 自註冊），
...
3. 兩個 `dcr/` server 變體已 build 過：
   ...
   make
   # 或單獨：
   go build -o bin/dcr-oauth-server            ./03-oauth-mcp/dcr/oauth-server
   go build -o bin/dcr-oauth-server-introspect ./03-oauth-mcp/dcr/oauth-server-introspect
   go build -o bin/dcr-oauth-client            ./03-oauth-mcp/dcr/oauth-client
   ```
...
兩台 server **共用同一個 AuthGate**，只是各自宣告不同的 `aud`。
...
- 開啟瀏覽器→AuthGate 同意頁→callback 完成。
...
```bash
# Server A
./bin/dcr-oauth-server-introspect \
  -addr :8095 \
  -resource http://localhost:8095/mcp \
  -auth-server http://localhost:8080 \
...
（`checkAudience`）功能一致。記得在 AuthGate 上幫兩台 RS 各自註冊一組
...
如果 RS 不檢 `aud`，那一張 AuthGate 簽出來的 token——只要 `iss` / `exp` /
簽章都對——就會被同一個 AuthGate 後面**所有** MCP server 接受，等於開後
